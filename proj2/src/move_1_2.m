classdef move_1_2 <handle
    %MOVE_1_2 此处显示有关此类的摘要
    %   此处显示详细说明
    ...
    methods(Access = public)
        ...
        function output = get_column(chess)
         ...
         function result = left_check(chess,chessboard)
            ...
            if chess.position_column ~= 1
                if (chessboard(chess.position_row(1,1),chess.position_column-1) == 0)&&(chessboard(chess.position_row(1,2),chess.position_column-1) == 0)
                    left = true;
                end
            end
            result = left;
      ...
      function result = up_check(chess,chessboard)
            ...
            if chess.position_row(1,1) ~= 1
                if chessboard(chess.position_row(1,1)-1,chess.position_column) == 0
                    up = true;
                end
            end
      ...
      function result = down_check(chess,chessboard)
        ...
        if chess.position_row(1,2) ~= 5
            if chessboard(chess.position_row(1,2)+1,chess.position_column) == 0
                down = true;
            end
        end
        result = down;
