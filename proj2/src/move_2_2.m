classdef move_2_2 <handle
    %MOVE_2_2 此处显示有关此类的摘要
    %   此处显示详细说明
    
    properties(Access = private)
        position_row   %2*1 size
        position_column   %2*1 size
    end
    ...
    methods(Access = public)
        ...
        function output = get_column(chess)
         ...
         function result = left_check(chess,chessboard)
            ...
            if chess.position_column(1,1) ~= 1
                if (chessboard(chess.position_row(1,1),chess.position_column(1,1)-1) == 0)&&(chessboard(chess.position_row(1,2),chess.position_column(1,1)-1) == 0)
                    left = true;
                end
            end
            result = left;
        ...
        function result = right_check(chess,chessboard)
            ...
            if chess.position_column(1,2) ~= 4
                if (chessboard(chess.position_row(1,1),chess.position_column(1,2)+1) == 0)&&(chessboard(chess.position_row(1,2),chess.position_column(1,2)+1) == 0)
                    right = true;
                end
            end
            result = right;
        ...
        function result = up_check(chess,chessboard)
            ...
            up = false;
            %up
            if chess.position_row(1,1) ~= 1
                if (chessboard(chess.position_row(1,1)-1,chess.position_column(1,1)) == 0)&&(chessboard(chess.position_row(1,1)-1,chess.position_column(1,2)) == 0)
                    up = true;
                end
            end
            result = up;
        ...
        function result = down_check(chess,chessboard)
            ...
            down = false;
            %down
            if chess.position_row(1,2) ~= 5
                if (chessboard(chess.position_row(1,2)+1,chess.position_column(1,1)) == 0)&&(chessboard(chess.position_row(1,2)+1,chess.position_column(1,2)) == 0)
                    down = true;
                end
            end
            result = down;
